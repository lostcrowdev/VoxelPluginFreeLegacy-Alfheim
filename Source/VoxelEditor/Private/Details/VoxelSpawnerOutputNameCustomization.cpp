	...
	IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	if (!Outputs)
	{
		...
		[
			FVoxelEditorUtilities::CreateText(VOXEL_LOCTEXT("Invalid Generator Outputs"), FSlateColor(FStyleColors::Error))
		];
		return;
	}
	...
	ComboBoxText = FVoxelEditorUtilities::CreateText(FText::FromName(Value), FSlateColor(ValuePtrPtr ? FSlateColor::UseForeground() : FStyleColors::Error));
